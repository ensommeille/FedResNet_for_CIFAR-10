...
def parse_args():
    ...
    parser.add_argument('--num_selected_clients', type=int, default=5, help='Number of selected clients per round')
    parser.add_argument('--sample_strategy', type=str, default='mixed', choices=['random', 'performance', 'mixed'],
                        help='Sampling strategy: random, performance, or mixed')
...
# 定义 ResNet18 模型（修改全连接层适配 CIFAR-10）
class ResNetFed(nn.Module):
    def __init__(self, pretrained=False):
        super(ResNetFed, self).__init__()
        self.model = models.resnet18(pretrained=pretrained)
        self.model.fc = nn.Linear(self.model.fc.in_features, 10)  # 修改输出层为 10 类
...
class Client:
    def __init__(self, client_id, data_indices):
        ...
        self.model = ResNetFed(pretrained=False).to(device)
        # 使用 weight_decay 提升正则化效果
...
def main():
    ...
    with tqdm(total=num_rounds, desc="Training Rounds") as pbar:
    ...
    file_name = f"client_{num_clients}_epoch_{local_epochs}_round_{num_rounds}_selected_{num_selected_clients}_lr_{learning_rate}_strategy_{sample_strategy}.png"
    ...
    result_file_name = f"client_{num_clients}_epoch_{local_epochs}_round_{num_rounds}_selected_{num_selected_clients}_lr_{learning_rate}_strategy_{sample_strategy}.json"
